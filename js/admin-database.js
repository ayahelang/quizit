...
(function (global) {
  ...
  function refreshConfig(cfg) {
    if (cfg && typeof cfg === 'object') {
      _cfgCache = cfg;
      global.__CBT_CONFIG__ = cfg;
      global.config = cfg;
    }
  }
  function getCfg() {
    return _cfgCache || global.__CBT_CONFIG__ || global.config || {};
  }
  function getAnonKey(c) {
    c = c || getCfg();
    return c.supabaseAnonKey || c.supabaseKey || c.anonKey || '';
  }
  function getSbUrl(c) {
    c = c || getCfg();
    return c.supabaseUrl || c.supabaseURL || '';
  }
  function sbEnabled() {
    const c = getCfg();
    return !!(getSbUrl(c) && String(getSbUrl(c)).trim() && getAnonKey(c) && String(getAnonKey(c)).trim());
  }
  function sbHeaders(extra) {
    const c = getCfg();
    const anon = getAnonKey(c);
  ...
  async function sbFetch(path, options) {
    options = options || {};
    const c = getCfg();
    const url = String(getSbUrl(c)).replace(/\/$/, '') + '/rest/v1/' + path;
